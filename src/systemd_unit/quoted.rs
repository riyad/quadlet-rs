    ...
    fn parse_escape_sequence(&mut self) -> Result<char, Error> {
        if let Some(c) = self.cur {
            let r = match c {
                ...
                c => {
                    return Err(Error::Unquoting(format!(
                        "expecting escape sequence, but found {c:?}."
                    )))
                }
            };
...
mod tests {
    ...
    mod quote_words {
        ...
        fn keeps_words_that_do_not_need_quoting() {
            ...
            assert_eq!(quote_words(words.into_iter()), "foo bar".to_string())
        }
        ...
        fn joins_words_with_space() {
            ...
            assert_eq!(quote_words(words.into_iter()), "foo bar baz".to_string())
        }
    }
